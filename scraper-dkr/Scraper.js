...
exports.Scraper = async(event, context) => {
    ...
    const launchArgs = {
        // some args for chromium, theres a million of them fuckers, heres a link, enjoy
    ...
    // --- Its all a bunch of methods below here ---
    ...
    try {
        ...
        let response = { statusCode: 400, Payload: `SCRAPER:: Doc type cannot be determined! Stopping scraper!` };
        if (!global_ScrapeDocType) return response;

        const browser = await puppeteer.launch(launchArgs).catch(rej => { throw rej });
...
// Test schema: Scraper({event: {}, context: AWS.Context})

// this.Scraper({ bucketName: "vinnyscrape-screenshots", scrapeUrl: "https://d8ev7ren9xjon.cloudfront.net/scraping/invoices" }, { awsRequestId: "invoicetest1234" });
// this.Scraper({ bucketName: "vinnyscrape-screenshots", scrapeUrl: "https://d8ev7ren9xjon.cloudfront.net/scraping/tables" }, { awsRequestId: "tabletest1234" });
// this.Scraper({ bucketName: "vinnyscrape-screenshots", scrapeUrl: "https://d8ev7ren9xjon.cloudfront.net/scraping/tables?checkbox=true" }, { awsRequestId: "tablechecktest1234" });
